...
class Model(object):
    def __init__(self, is_training, config, filename):
        ...
        with tf.device("/cpu:0"):
            ...
            keyword_inputs = tf.nn.embedding_lookup(embedding, self._input_word)#keyword_inputs变成batch_size * _input_word length * embedding_size
        ...
        with tf.variable_scope("RNN"):
            entropy_cost = []
            for time_step in range(num_steps):     #对每个t
                vs = []
                for s2 in range(config.num_keywords):   #对每个关键词j
                    with tf.variable_scope("RNN_attention"):
                        if time_step > 0 or s2 > 0: tf.get_variable_scope().reuse_variables()
                        u = tf.get_variable("u", [size, 1])     #u就是一个需要优化的量
                        w1 = tf.get_variable("w1", [size, size])
                        w2 = tf.get_variable("w2", [config.word_embedding_size, size])
                        b = tf.get_variable("b1", [size])

                        vi = tf.matmul(tf.tanh(tf.add(tf.add(   #Vi表示vaT tanh(Waht−1 + Uatopicj)
                            tf.matmul(output_state, w1),        #w1表示Wa,output_state表示h(t-1)
                            tf.matmul(keyword_inputs[:, s2, :], w2)), b)), u)   # w2表示Ua
                        vs.append(vi * gate[:, s2:s2 + 1])      #vi * gate[:, s2:s2 + 1]得到g(t,j),即attention score

                self.attention_vs = tf.concat(vs, axis=1)
                prob_p = tf.nn.softmax(self.attention_vs)   #the probability of the next word y(t)

                gate = gate - (prob_p / phi_res)    #gate表示C(t,j),prob_p表示a(t,j),phi_res表示fai(j)

                atten_sum += prob_p * self._mask[:, time_step:time_step + 1]    #atten_sum表示T(t) 即context vector
